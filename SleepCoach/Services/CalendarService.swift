...
final class CalendarService: CalendarProviding {
    ...
    private let legacyGymEventIDKey = "sleepCoachGymEventID"
    private let gymEventReceiptsKey = "sleepCoachGymEventReceipts"
    private var eventStoreObserver: NSObjectProtocol?
    private var eventStoreChangeHandler: (() -> Void)?
    private static let legacyGymEventTitle = "Gym · Sleep Coach plan"
    private static let gymEventNoteMarker = "Created by Sleep Coach."
            ...
            if let existingReceipt {
                ...
                case .ambiguous:
                    failures.append(CalendarEventOperationFailure(
                        ...
                        calendarTitle: displayTitle(for: destination),
                        role: role,
                        message: "More than one calendar item matches the saved identity, so Sleep Coach did not update or duplicate it. Use Undo after resolving the duplicate."
                    ))
                    continue
                }
            } else {
            ...
            do {
                ...
                guard !eventIdentifier.isEmpty || externalIdentifier != nil else {
                    ...
                    failures.append(CalendarEventOperationFailure(
                        ...
                        calendarTitle: displayTitle(for: destination),
                        role: role,
                        message: "Calendar did not return a usable identifier, so Sleep Coach removed the untrackable event."
                    ))
                    continue
                }
                let receipt = CalendarEventReceipt(
                ...
                if persistReceipts(activeReceipts) {
                    ...
                    do {
                        ...
                        failures.append(CalendarEventOperationFailure(
                            ...
                            calendarTitle: displayTitle(for: destination),
                            role: role,
                            message: "Sleep Coach couldn’t securely save the Undo receipt, so it rolled back the Calendar event."
                        ))
                    } catch {
                        writtenReceipts.append(receipt)
                        failures.append(CalendarEventOperationFailure(
                    ...
                    failures.append(CalendarEventOperationFailure(
                        ...
                        calendarTitle: displayTitle(for: destination),
                        role: role,
                        message: "The event was updated, but Sleep Coach couldn’t securely refresh its Undo receipt. The prior exact receipt was retained."
                    ))
                }
            } catch {
                failures.append(CalendarEventOperationFailure(
        ...
        for receipt in obsoleteReceipts {
            ...
            case .ambiguous:
                failures.append(CalendarEventOperationFailure(
                    ...
                    calendarTitle: receipt.calendarTitle,
                    role: receipt.role,
                    message: "The previous app-owned event has an ambiguous external identity. Sleep Coach retained its Undo receipt and did not remove any matching item."
                ))
                continue
            }
            do {
    ...
    func verifyGymEvents(
        ...
        for receipt in receipts {
            ...
            case .ambiguous:
                ...
                failures.append(CalendarEventOperationFailure(
                    ...
                    calendarTitle: receipt.calendarTitle,
                    role: receipt.role,
                    message: "More than one calendar item matches the saved external identifier. Sleep Coach did not choose between them."
                ))
                continue
            }
        ...
        if !persistReceipts(verified + missing) {
            failures.append(CalendarEventOperationFailure(
                ...
                calendarTitle: "Calendar",
                role: .detailed,
                message: "Sleep Coach verified the events but couldn’t securely refresh their Undo receipts."
            ))
        }
        return CalendarEventVerificationResult(
        ...
        for receipt in receipts {
            ...
            case .ambiguous:
                ...
                failures.append(CalendarEventOperationFailure(
                    ...
                    calendarTitle: receipt.calendarTitle,
                    role: receipt.role,
                    message: "More than one calendar item matches the saved external identifier. Sleep Coach did not remove either one."
                ))
                continue
            }
            do {
    ...
    private static func hasOwnedMarker(title: String?, note: String?) -> Bool {
        ...
        return note?.contains(gymEventNoteMarker) == true
    }
