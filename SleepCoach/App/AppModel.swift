...
final class AppModel: ObservableObject {
    ...
    private func saveCalendarPreferences() {
        ...
        if privateStateStore.set(data, forKey: Self.calendarPreferencesKey) {
            ...
            notice = "Sleep Coach couldn’t securely save your calendar choices. Keep the app open and try again after unlocking your iPhone."
        }
    }
        ...
        if current.calendarEventApplied {
            do {
                ...
                messages.append(contentsOf: result.failures.map {
                    "Sleep Coach couldn’t verify \($0.role.displayTitle.lowercased()) in \($0.calendarTitle): \($0.message)"
                })
            } catch {
