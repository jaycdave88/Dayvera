...
final class AlarmService: AlarmScheduling {
    ...
    private let alarmIDKey = LegacyCompatibility.wakeAlarmID
    private let alarmIDsKey = LegacyCompatibility.wakeAlarmIDs
...
enum AlarmServiceError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .postScheduleInspectionFailed(let detail):
            "The new alarm may have been created, but Dayvera could not verify replacement cleanup. Use Undo before applying again. \(detail)"
        case .replacementCleanupFailed(let detail):
            "An older Dayvera alarm could not be removed, so the new alarm was rolled back. Use Undo before applying again. \(detail)"
        case .replacementRollbackFailed(let cleanupDetail, let rollbackDetail):
            "An older Dayvera alarm and the new replacement could not both be removed. Use Undo before applying again. \(cleanupDetail) \(rollbackDetail)"
        case .cleanupFailed(let detail):
            "One or more Dayvera alarms could not be removed. Try Undo again. \(detail)"
        }
    }
}
